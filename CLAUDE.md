...
- **Dates are NJ-local**: Render servers run UTC, so naive `date.today()` rolls to tomorrow at 8 PM EDT. Always use `local_today()` (striper_tides.py) in Python. In the template, use `todayStr()`/`njDay()`/`dsOf()` for date strings — never `new Date().toISOString()`.
